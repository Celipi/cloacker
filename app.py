...
def redirect_link(short_id):
    # Obter todos os parâmetros da URL original, exceto access_code
    ...
    if ab_test:
        ...
        if passed_filter:
            ...
            # Adicionar parâmetros originais à URL escolhida
            parsed_url = urlparse(chosen_url)
            query_params = dict(parse_qsl(parsed_url.query))
            query_params.update(original_params)
            new_query = urlencode(query_params)
            final_url = parsed_url._replace(query=new_query).geturl()
            ...
            response = make_response(redirect(final_url))
            response.set_cookie(cookie_name, 'true', max_age=30*24*60*60)  # Cookie válido por 30 dias
            response.set_cookie(f'chosen_url_{short_id}', final_url, max_age=30*24*60*60)
            return response
        else:
            return render_template('block.html', safe_url=ab_test['safe_url'])
    ...
    if request.cookies.get(cookie_name) == 'true':
        parsed_url = urlparse(link['offer_url'])
        query_params = dict(parse_qsl(parsed_url.query))
        query_params.update(original_params)
        new_query = urlencode(query_params)
        final_url = parsed_url._replace(query=new_query).geturl()
    ...
    if passed_filter:
        # Adicionar parâmetros originais à URL de oferta
        parsed_url = urlparse(link['offer_url'])
        query_params = dict(parse_qsl(parsed_url.query))
        query_params.update(original_params)
        new_query = urlencode(query_params)
